...
void ZZDiBosonAnalysis::FillOutputList()
{
  ...
  /* Add Global histograms(AGREGAR HISTOGRAMAS GLOBALES) 
Debe registrar el histograma en la lista de salida de su selector, pasándolo a GetOutputList () -> Add (hist), idealmente justo después de haberlo creado con el nuevo TH1F (...). Si no lo agrega a la lista de resultados, PROOF no puede saber que este histograma es el resultado del análisis.(Consigue de la lista de salida los objetos a ser transferidos de vuelta al usuario)*/
 ...
 // Add  Lepton histograms(Agregar histogramas de Lepton)
...
void ZZDiBosonAnalysis::WriteHistograms()
{
  ...
  // Write Global histograms(Preescribiendo esos histogramas)
  hist_mLL1->Write();//Abrir esos  histogramas 
  hist_mLL2->Write();
